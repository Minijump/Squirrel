...
def test_create_table_from_csv(mock_project):
    ...
    with patch('os.getcwd', return_value=mock_project):     
        response = client.post("/project/create_table", data={"data_source_dir": "mock_source_csv", 'project_dir': mock_project})
        ...
        with open(pipeline_path, 'r') as file:
            lines = file.readlines()
            assert any("df = pd.read_csv(" in line for line in lines), "Table not created in pipeline"

        assert response.context.get("table"), "Response does not contain a table"
        assert "mock_name" in response.context.get("table"), "Table should contain a 'mock_name' column"
        assert "mock_price" in response.context.get("table"), "Table should contain a 'mock_price' column"
