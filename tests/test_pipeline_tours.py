...
class TestPipelineTours:
    ...
    def test_change_action_order(self, server, browser, reset_projects):
        ...
        original_third_action = action_elements[2]
        original_third_action_text = original_third_action.text
        
        tour.move_action(original_third_action, original_first_action)
        ...
        assert action_elements_after_leaving_page[0].text == original_third_action_text, "New action order is not saved; first action is not correct"
        assert action_elements_after_leaving_page[2].text == original_first_action_text, "New action order is not saved; third action is not correct"
